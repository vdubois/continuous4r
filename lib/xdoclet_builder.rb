...
class XdocletBuilder
  ...
  def build(project_name, auto_install, proxy_option)
    ...
    files << Dir.glob("app/**/*.html.erb")
    files << Dir.glob("app/**/*.rhtml")
    ...
    files.each do |file|
      ...
      html_file = File.open("#{Continuous4r::WORK_DIR}/xdoclet/#{file.gsub(Regexp.new("#{'.'.gsub(/\./,"\\.")}/"),'').gsub(/\//,'_')}.html", "w")
      html_global_code = "<html><head><title>Source code for #{file.gsub(Regexp.new("#{'.'.gsub(/\./,"\\.")}/"),'')}</title>"
      html_global_code = html_global_code + "<style>#{File.read("#{File.dirname(__FILE__)}/site/style/coderay.css")}</style>"
      html_global_code = html_global_code + "<body><span style='font-family: Arial, Verdana, Helvetica; color: brown; font-weight: bold; font-size: 18px;'>Source code for #{file.gsub(Regexp.new("#{'.'.gsub(/\./,"\\.")}/"),'')}</span><br/><br/>#{html_code}</body></html>"
